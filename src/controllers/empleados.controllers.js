...
export const getEmpleado = async (req, res) => {
    const [rows] = await pool.query('SELECT * FROM colaborador WHERE rut_colaborador = ?', [req.params.rut])
    res.json(rows[0])
    try {
    } catch (error) {
        return res.status(500).json({
...
export const patchEmpleados = async (req, res) => {
    try {
        ...
        const {nombre_colaborador,
               apellido_colaborador,
               telefono_colaborador,
               correo_electronico,
               contraseña} = req.body
    
        const [result] = await pool.query('UPDATE colaborador SET nombre_colaborador = IFNULL(?, nombre_colaborador), apellido_colaborador = IFNULL(?, apellido_colaborador), telefono_colaborador = ifnull(?, telefono_colaborador) , correo_electronico = IFNULL(?, correo_electronico), contraseña = IFNULL(?, contraseña) WHERE rut_colaborador = ?', [nombre_colaborador, apellido_colaborador, telefono_colaborador, correo_electronico, contraseña, rut])
        ...
        const [rows] = await pool.query('SELECT*FROM colaborador WHERE rut_colaborador = ?', [rut])
        res.json(rows[0])
    } catch (error) {
        return res.status(500).json({
            message: 'Algo ha salido mal'
...
export const deleteEmpleados = async (req, res) =>{
   try {
    const [result] = await pool.query('DELETE FROM colaborador WHERE rut_colaborador = ?', [req.params.rut])
